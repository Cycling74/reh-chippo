...
EditorContainer::EditorContainer (CustomAudioProcessor* const p, RNBO::CoreObject& rnboObject)
    : _audioProcessor (p)
    , rnboProcessor (p)
    , _rnboObject (rnboObject)
    , appProperties (p->appProperties)
    , presetTree (p->presetTree)
    ...
    , _parameterInterface (_rnboObject.createParameterInterface (RNBO::ParameterEventInterface::SingleProducer, this))
{
    ...
    setSizeFromSequencers();

    auto props = appProperties.getCommonSettings (true);
    auto initScale = props->getDoubleValue ("scale", 1.0);
    setScale ((float)initScale);
}

void EditorContainer::setScale (float newScale)
{
    scale = newScale;
    auto props = appProperties.getCommonSettings (true);
...
void EditorContainer::resized()
{
    ...
    zoomButton.setBounds (5, 95, 50, 20);

    {
        ...
        sliders[Sliders::rootNote]->setBounds (runToggle->getRight() + 90, topRowControlsY, 105, 35);
        scaleBox->setBounds (sliders[Sliders::rootNote]->getRight() + controlOffset, topRowControlsY, 131, 35);
        sliders[Sliders::stepLength]->setBounds (scaleBox->getRight() + controlOffset, topRowControlsY, 55, 35);
    ...
    {
        auto togBnds = getLocalBounds().withTrimmedTop (815).removeFromRight (750);
        for (auto i = 0; i < seqVisToggles.size(); ++i)
        {
            auto scoot = 0;
            if (i == 3) // move snare over a little
                scoot = 7;
            seqVisToggles[i]->setBounds (togBnds.getX() + (i * 140) + scoot, togBnds.getY(), 25, 25);
        }
    }

    auto sequencerBounds = getLocalBounds().withTrimmedTop (850);
    auto indicatorBounds = sequencerBounds;
    indicatorBounds.removeFromLeft (35); // label space
    sequencerBounds.removeFromTop (35); // space for indicator
    ...
    for (auto& s : sequencers)
    {
        if (s->isVisible())
        {
            s->setBounds (sequencerBounds.removeFromTop (25));
            bottom = s->getBottom();
        }
    }
    seqStepIndicator.setBounds (indicatorBounds.withBottom (bottom));
}

void EditorContainer::handleMessageEvent (const RNBO::MessageEvent& event)
{
...
void EditorContainer::sendSequencerValues (SequencerComponent& seq, StringRef inputTag)
{
...
void EditorContainer::setupSliders()
{
...
void EditorContainer::setupSequencers()
{
    auto parameters = rnboProcessor->getParameters();
...
void EditorContainer::setupToggles()
{
    ...
    imgOn  = ImageCache::getFromMemory (BinaryData::chiptog1_png, BinaryData::chiptog1_pngSize);

    auto seqTree = presetTree.getChildWithName(sequencerVisIdt);
    auto index = 0;
    for (auto& i : SeqButtons::genIdts)
    {
        auto t = seqVisToggles.add (new nlt::ValueTreeToggleButton (seqTree, i));
        t->setImages (false,
                      true,
                      true,
                      imgOff,
        ...
        auto l = seqVisLabels.add (new Label ("", i.toString()));
//        l->setJustificationType(Justification::right);
        l->setColour (Label::textColourId, Colours::black);
        l->attachToComponent (t, true);
        l->setFont (20.0f);
        addAndMakeVisible (l);


        vtCallbacks.add (seqTree, i, [this, index](bool show)
                         {
                             sequencers[index]->setVisible (show);
                             setSizeFromSequencers();
                         });
        index++;
    }
...
void EditorContainer::setupButtons()
{
    ...
    zoomButton.setColour (TextButton::buttonOnColourId, { 231, 232, 233 });
    zoomButton.setMouseCursor (MouseCursor::PointingHandCursor);
    zoomButton.setClickingTogglesState (true);
    zoomButton.onClick = [this]()
    {
        PopupMenu scalingMenu;

        auto addItem = [&] (float newScale)
        {
            auto currentScale = approximatelyEqual (scale, newScale);
            scalingMenu.addItem (
                String (newScale * 100.0f) + "%", true, currentScale, [this, newScale]() { setScale (newScale); });
        };

        for (auto s = 0; s <= 9; s++)
            addItem (s * 0.1f + 0.20f);

        for (auto s = 1; s < 9; s++)
            addItem (s * 0.1f + 1.0f);

        scalingMenu.showMenuAsync (PopupMenu::Options().withTargetComponent (&zoomButton));
    };
    addAndMakeVisible (zoomButton);
}

void EditorContainer::setupTooltips()
{
    tooltipWindow->setMillisecondsBeforeTipAppears (1200);
    zoomButton.setTooltip("Toggle between window sizes");
...
void EditorContainer::setSizeFromSequencers()
{
    auto height = 855; // height with no sequencers visible
    for (auto& s : sequencers)
    {
        if (s->isVisible())
            height += 25;
    ...
    if (height > 855)
        height += 38; // this makes room for the indicator
