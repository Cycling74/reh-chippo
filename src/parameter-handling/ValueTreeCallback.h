...
class ValueTreeCallback : public NO_ACCESS::ValueTreeCallbackBase,
...
private:
    ...
    ValueTreeCallback (ValueTree tree, const juce::Identifier& _property, Fn&& callbackToUse, bool init = true)
        : valueTree (tree), property (_property), callback (NLT_FWD (callbackToUse))
    {
...
public:
    ...
    void add (ValueTree tree, const juce::Identifier& property, Fn&& callbackFn, bool init = true)
    {
        callbacks.add (createCallback (
            tree, property, std::move (DeduceTypeFromLambdaFunctionArg::functionFromLambda (NLT_FWD (callbackFn))), init));
    }
...
private:
    ...
    template <typename Type>
    static auto createCallback (ValueTree tree, const juce::Identifier& property, std::function<void (Type)>&& callback, bool init)
    {
